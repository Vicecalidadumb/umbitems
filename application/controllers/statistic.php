...
class Statistic extends CI_Controller {
    ...
    public function data() {
        $array_return = array();
        $array_data = array_sql();
        foreach ($array_data as $key => $array) {
            //echo $key.'<br>';
            //$array_return[] = $key;
            foreach ($array as $key_2 => $array_u) {
                //echo '-----'.$array_u.'<br>';
                $array_return[] = $key . ' - ' . $array_u;
                //echo "INSERT INTO siei_test(USUARIO_ID,EVALUACION_ID,PREGUNTA_ID,RESPUESTA) VALUES($key,1,".($key_2+1).",$array_u); <br>";
            }
        }
        //echo '<pre>'.print_r(array_sql(),true).'</pre>';
        echo '<pre>' . print_r($array_return, true) . '</pre>';
    }

    public function p1($ID_prueba = 1) {

        //$data['roles'] = get_dropdown($this->user_model->get_all_roles(), 'ID_TIPO_USU', 'NOM_TIPO_USU');
        //$data['states'] = get_array_states();

        $data['prueba'] = $this->statistic_model->get_prueba($ID_prueba);

        ////////////

        $data['ciudades'] = $this->statistic_model->get_all_cuidades($ID_prueba);
        $data['ciudades_prueba'] = $this->statistic_model->get_all_cuidades_porprueba($ID_prueba);

        /////////////  	


        $data['title'] = 'Estadisticas';
        $data['content'] = 'statistic/p1';
        $this->load->view('template/template', $data);
    }
